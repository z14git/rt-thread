...
// Check https://github.com/olikraus/u8g2/wiki/u8g2setupcpp for all supported
// devices U8G2_SSD1306_128X64_NONAME_F_HW_I2C u8g2(U8G2_R0, U8X8_PIN_NONE);
...
static const char *usb_test_explain = "用USB线连接电脑\n"
                                      "连接后，不要动鼠标,\n"
                                      "观察电脑屏幕上的\n"
                                      "鼠标是否左右移动。\n"
                                      "若鼠标左右移动，\n"
...
static const char *db9_test_explain = "用串口线连接电脑\n"
                                      "三档开关拔到左边\n"
                                      "电脑打开串口调试工具\n"
                                      "波特率设为115200\n"
                                      "单片机会每隔3秒\n"
...
{
    char buf[64]; // there are at most 8 chinese glyphs per line, max buf size
                  // is 8*3 = 24

    uint8_t total_lines; // the total number of lines in the story
    uint8_t i;           // loop variable for the lines
    uint8_t
            line_cnt; // number of lines to draw, usually equal to lines_per_draw
    uint8_t start_line;     // topmost visible line, derived from top_window_pos
    uint8_t lines_per_draw; // how many lines to draw on the screen, derived
                            // from font and display height
    uint16_t glyph_height;  // height of the glyphs

    uint16_t
        top_window_pos; // defines the display position in pixel within the text
    u8g2_uint_t
           top_offset; // offset between the first visible line and the display
    int8_t event;
...
static void board_test_thread_entry(void *parameter)
{
    ...
    while (1) {
        current_selection = u8g2.userInterfaceSelectionList("选择测试项目",
                                                            current_selection,
                                                            string_list);
