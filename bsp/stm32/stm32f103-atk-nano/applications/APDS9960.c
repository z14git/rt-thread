...
#define APDS9960_I2CBUS_NAME "i2c1"

static struct rt_i2c_bus_device *apds9960_i2c_bus;
...
static int apds9960_hw_init(void)
{
    apds9960_i2c_bus = rt_i2c_bus_device_find(APDS9960_I2CBUS_NAME);
    if (RT_NULL == apds9960_i2c_bus)
    {
        return -RT_ERROR;
    }
    return RT_EOK;
}
INIT_COMPONENT_EXPORT(apds9960_hw_init);
...
int wireWriteDataByte(unsigned char reg, unsigned char val)
{
    ...
    msgs.addr = APDS9960_I2C_ADDR;
    msgs.flags = RT_I2C_WR;
    msgs.buf = buffer;
    msgs.len = sizeof(buffer);

    if(rt_i2c_transfer(apds9960_i2c_bus, &msgs, 1) == 1) {
        return 1; // true
    } else {
        return 0; // false
    }
