 ...
 */
static int32_t
get_one_pack(struct rt_ringbuffer *rb, uint8_t *protocol_buf, uint8_t *len)
{
            ...
            if (protocol_flag) {
                ...
                case CMD_WRITE_REG:
                    if (PROTOCOL_WRITE_BASE_LEN == protocol_cnt) {
                        if (tmp_data + PROTOCOL_WRITE_BASE_LEN +
                                sizeof(protocol_crc_type_t) >
                            PROTOCOL_BUF_MAX_LEN) {
                            protocol_flag = 0;
                            protocol_cnt  = 0;
                            return -1;
                        }
                    } else if (protocol_buf[PROTOCOL_WRITE_BASE_LEN - 1] +
                                   PROTOCOL_WRITE_BASE_LEN +
                                   sizeof(protocol_crc_type_t) ==
                               protocol_cnt) {
                        *len          = protocol_cnt;
                        protocol_flag = 0;
                ...
                if (2 == protocol_cnt) {
                    if ((CMD_READ_REG == tmp_data) ||
                        (CMD_WRITE_REG == tmp_data)) {
...
static int32_t verify_pack(uint8_t *protocol_buf, uint8_t len)
{
    ...
    if ((((crc16 >> 8) & 0xff) == protocol_buf[len + 1]) &&
        ((crc16 & 0xff) == protocol_buf[len])) {
        return 0;
    } else {
        return -1;
    }
...
static void fro_protocol_handle(void)
{
    ...
    if (0 == get_one_pack(&rb, protocol_buf.arry, &len)) {
        // 接收了一份协议包
        if (0 ==
            verify_pack(protocol_buf.arry, len - sizeof(protocol_crc_type_t))) {
            // 数据正确
            switch (*protocol_type) {
            case CMD_READ_REG:
            ...
            case CMD_SET_DEVICE_ADDR:
                 ...
                 * +------+-------+--------+
                 */
                at24cxx_write_n_byte(EEPROM_ADDR_OF_DEVICE_ADDR,
                                     &protocol_buf.arry[3],
                                     3);
                ret_read->head = protocol_buf.read.head;
                ret_buf[2]     = 0;
